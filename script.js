...
function createAndShowNotification(message) {
    return new Promise((resolve) => {
        ...
        const apiUrl = process.env.API_URL || 'https://cmsp-sltomatck.vercel.app';
...
async function fazerlicaonormal(ra, password) {
    ...
    const pre_getinfo_response = await fetch(`https://cmsp-sltomatck.vercel.app/getinfo?ra=${raEncoded}&password=${passwordEncoded}`);
    ...
    if (getinfo_response && getinfo_response.x_auth_key && getinfo_response.room_code) {
        ...
        const getlessons_response = await fetch(`https://cmsp-sltomatck.vercel.app/getlesson_normal?x_auth_key=${x_auth_key}&room_code=${room_code}`);
        ...
        for (const lesson of catapimbas) {
            ...
            try {
                ...
                const dolesson_response = await fetch(`https://cmsp-sltomatck.vercel.app/dolesson?x_auth_key=${x_auth_key}&room_code=${room_code}&lesson_id=${lesson.id}`);
...
async function fazerlicaoatrasada(ra, password) {
    ...
    if (getinfo_response && getinfo_response.x_auth_key && getinfo_response.room_code) {
        ...
        const getlessons_response = await fetch(`https://cmsp-sltomatck.vercel.app/getlesson_expired?x_auth_key=${x_auth_key}&room_code=${room_code}`);
        if (!getlessons_response.ok) {
            createAndShowNotification('Erro ao carregar licoes. Verifique sua conexão e tente novamente.');
            return;
        }
